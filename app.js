  ...
  return new Promise(resolve => {
    ...
    recorder.onstop = async event => {
      try {
        ...
        log(error.message || 'Unable to finalize the recording.', 'bad');
        resetAfterFinalize();
      } finally {
        resolve();
      }
    ...
    try {
      ...
      log('Recorder could not flush a final data chunk before stopping.', 'warn');
    }
    recorder.stop();
  });
}
...
async function finalizeRecording() {
  ...
  els.packageStat.textContent = 'Preparing ZIP';
  els.outputLine.textContent = `${videoName} - ${formatBytes(blob.size)} - ${formatTime(durationSeconds)}`;
  log(`Recording captured: ${videoName}`, 'good');
  prepareRecordingExport(recording);
}

async function prepareRecordingExport(recording) {
  try {
    await remuxRecordingIfNeeded(recording);
    attachRecordingToPlayer(recording);
    renderFileList(recording);
    els.outputLine.textContent = `${recording.videoName} - ${formatBytes(recording.sizeBytes)} - ${formatTime(recording.durationSeconds)}`;
    ...
    els.outputLine.textContent = `${recording.videoName} - ${formatBytes(recording.sizeBytes)} - ${formatTime(recording.durationSeconds)}`;
    log(`Recording export ready: ${recording.videoName}`, 'good');
  } catch (error) {
    els.downloadBtn.disabled = true;
    els.packageStat.textContent = 'Video only';
    log(error.message || 'ZIP package could not be created. The video preview/download item is still available below.', 'bad');
  }
}
